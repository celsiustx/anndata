...
def read_dataframe(group, dask: bool = False) -> pd.DataFrame:
    ...
    if dask:
        ...
        # NOTE: This was previously only set if the index was not `_index`,
        # but leaving it as None is not entirely honest, and causes diffs to fail,
        # vs autogenerated things that are more honest.
        if idx_key != "_index":
